...
class Controller(object):
    ...
    def profile_1(self):
        self.speed_limits()
        if self._left_stick_y <= 0.04 and self._left_stick_y >= -0.04:
            self._left_stick_y = 0.0
        if self._right_stick_x <= 0.05 and self._right_stick_x >= -0.05:
            self._right_stick_x = 0.0
        if self._r3 != 0:
    ...
    def profile_2(self):
        ...
        if self._left_stick_x <= 0.05 and self._left_stick_x >= -0.05:
            self._left_stick_x = 0.0
        if self._l3 != 0:
            if not self._inverted_angular_lock:
